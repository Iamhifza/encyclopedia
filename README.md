...
**188 entries · 877 typed relationships · 21 domains · 10 comparison pages ·
8 learning paths**

Entries come in two depths. **96 full entries** carry the complete treatment —
500 to 1,200 words across ten-plus sections, with diagrams, worked numbers and
primary sources. **92 seed entries** are lookup records: canonical name, aliases,
one-line definition and position in the concept graph, awaiting expansion. Both
appear in search, the A-Z index, topic pages and the graph; the
[coverage page](https://Iamhifza.github.io/encyclopedia/meta/coverage/)
reports the ratio per domain rather than hiding it.

Every one of the 21 domains has entries. Inference and agent engineering are
deliberately the deepest, since that is where the vocabulary is densest and worst
documented elsewhere.

## Growing it

The corpus is designed to be extended in batches. A queue of terms lives in
`content/backlog.yaml`; each carries a definition and its graph edges, so
generating them produces real records rather than stubs.

```bash
enc batch --limit 20            # create the next twenty seed entries
enc todo                        # which seeds most deserve the full treatment
enc promote "Beam Search"       # flip a seed to full and get the skeleton
```
